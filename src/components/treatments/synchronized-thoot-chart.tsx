// src/components/treatments/synchronized-tooth-chart.tsx
'use client';

import React, { useState, useCallback, useMemo } from 'react';
import { cn } from '@/lib/utils';
import { Info, ZoomIn, ZoomOut, RotateCcw, Link, Unlink } from 'lucide-react';
...
// Mapping between upper and lower teeth (antagonist teeth)
const TOOTH_PAIRS: Record<number, number> = {
  // Upper to Lower mapping
  1: 32, 2: 31, 3: 30, 4: 29, 5: 28, 6: 27, 7: 26, 8: 25,
  9: 24, 10: 23, 11: 22, 12: 21, 13: 20, 14: 19, 15: 18, 16: 17,
  // Lower to Upper mapping
  17: 16, 18: 15, 19: 14, 20: 13, 21: 12, 22: 11, 23: 10, 24: 9,
  25: 8, 26: 7, 27: 6, 28: 5, 29: 4, 30: 3, 31: 2, 32: 1
};

const TOOTH_POSITIONS = {
  // Upper jaw - adjusted for better spacing
  1: { x: 720, y: 120, quadrant: 1, type: 'molar' },
  2: { x: 670, y: 115, quadrant: 1, type: 'molar' },
  3: { x: 620, y: 110, quadrant: 1, type: 'molar' },
  4: { x: 570, y: 105, quadrant: 1, type: 'premolar' },
  5: { x: 520, y: 100, quadrant: 1, type: 'premolar' },
  6: { x: 470, y: 95, quadrant: 1, type: 'canine' },
  7: { x: 420, y: 90, quadrant: 1, type: 'incisor' },
  8: { x: 370, y: 85, quadrant: 1, type: 'incisor' },
  
  9: { x: 320, y: 85, quadrant: 2, type: 'incisor' },
  10: { x: 270, y: 90, quadrant: 2, type: 'incisor' },
  11: { x: 220, y: 95, quadrant: 2, type: 'canine' },
  12: { x: 170, y: 100, quadrant: 2, type: 'premolar' },
  13: { x: 120, y: 105, quadrant: 2, type: 'premolar' },
  14: { x: 70, y: 110, quadrant: 2, type: 'molar' },
  15: { x: 20, y: 115, quadrant: 2, type: 'molar' },
  16: { x: -30, y: 120, quadrant: 2, type: 'molar' },
...
// Component for individual tooth
const ToothSVG: React.FC<{
  ...
  readOnly?: boolean;
  showLabel?: boolean;
}> = ({ number, position, status, isSelected, isPaired, onToothClick, readOnly, showLabel }) => {
  ...
  return (
    ...
    >
      ...
      <path
        d={toothPath}
        fill={fillColor}
        stroke={isSelected ? '#ff5722' : isPaired ? '#3b82f6' : '#666'}
        strokeWidth={isSelected ? '2' : isPaired ? '2' : '1'}
        className={cn(
...
export const SynchronizedToothChart: React.FC<SynchronizedToothChartProps> = ({
  selectedTeeth = [],
  toothData = [],
  onSelectionChange,
  autoSyncUpperLower = true,
  readOnly = false,
  showLabels = true,
  showQuadrants = true,
  className,
  ...
  const [syncEnabled, setSyncEnabled] = useState(autoSyncUpperLower);
  ...
  // Check if a tooth is paired (its antagonist is selected)
  const isPaired = useCallback((toothNumber: number): boolean => {
    const pair = TOOTH_PAIRS[toothNumber];
    return pair ? selectedTeethMap.has(pair) : false;
  }, [selectedTeethMap]);

  // Handle tooth click with auto-sync
  const handleToothClick = useCallback((toothNumber: number) => {
    ...
    if (existingTooth) {
      // Remove tooth
      newSelection = newSelection.filter(t => t.number !== toothNumber);
      
      // If sync is enabled, also remove the paired tooth
      if (syncEnabled) {
        const pairedTooth = TOOTH_PAIRS[toothNumber];
        if (pairedTooth) {
          newSelection = newSelection.filter(t => t.number !== pairedTooth);
        }
      }
    } else {
      // Add tooth
      const newTooth: SelectedTooth = { number: toothNumber, surfaces: [] };
      newSelection.push(newTooth);
      
      // If sync is enabled, also add the paired tooth
      if (syncEnabled) {
        const pairedTooth = TOOTH_PAIRS[toothNumber];
        if (pairedTooth && !selectedTeethMap.has(pairedTooth)) {
          const pairedToothData = toothDataMap.get(pairedTooth);
          if (!pairedToothData || pairedToothData.status !== 'missing') {
            newSelection.push({ number: pairedTooth, surfaces: [] });
          }
        }
      }
    }
  ...
  }, [readOnly, selectedTeeth, selectedTeethMap, toothDataMap, syncEnabled, onSelectionChange]);
    ...
    <div className={cn('w-full', className)}>
      ...
      <div className="flex items-center justify-between mb-4 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
        <div className="flex items-center gap-2">
          ...
          <div className="ml-4 flex items-center gap-2">
            <button
              ...
              title="Sincronizare Superior/Inferior"
            >
              {syncEnabled ? <Link className="w-4 h-4" /> : <Unlink className="w-4 h-4" />}
              <span className="text-sm font-medium">
                {syncEnabled ? 'Sincronizare Activă' : 'Sincronizare Dezactivată'}
              </span>
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
          ...
          <span>Click pentru selectare</span>
          {syncEnabled && (
            <span className="text-blue-600 dark:text-blue-400 font-medium">
              • Antagoniștii se selectează automat
            </span>
          )}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 overflow-hidden">
        ...
        >
          <g transform={`scale(${zoom})`}>
            ...
            <rect x="-50" y="50" width="800" height="150" fill="url(#upperJawBg)" rx="10" />
            ...
            {showQuadrants && (
              <g opacity="0.2">
                ...
                <text x="520" y="45" fontSize="12" fill="#666" textAnchor="middle">Cadran 1</text>
                <text x="170" y="45" fontSize="12" fill="#666" textAnchor="middle">Cadran 2</text>
            ...
            {/* Jaw labels */}
            <text x="345" y="70" fontSize="14" fill="#333" textAnchor="middle" className="font-semibold">
              Maxilar Superior
            </text>
            ...
            {/* Connection lines for paired teeth */}
            {syncEnabled && selectedTeeth.map(tooth => {
              const pairedTooth = TOOTH_PAIRS[tooth.number];
              if (pairedTooth && selectedTeethMap.has(pairedTooth)) {
                const pos1 = TOOTH_POSITIONS[tooth.number as keyof typeof TOOTH_POSITIONS];
                const pos2 = TOOTH_POSITIONS[pairedTooth as keyof typeof TOOTH_POSITIONS];
                if (pos1 && pos2) {
                  return (
                    <line
                      key={`pair-${tooth.number}-${pairedTooth}`}
                      x1={pos1.x}
                      y1={pos1.y}
                      x2={pos2.x}
                      y2={pos2.y}
        ...
        <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
          ...
          <div className="flex flex-wrap gap-2">
            {selectedTeeth.sort((a, b) => a.number - b.number).map(tooth => {
              const paired = TOOTH_PAIRS[tooth.number];
              const hasPair = paired && selectedTeethMap.has(paired);
...
export default SynchronizedToothChart;