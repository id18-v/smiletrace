// src/components/treatments/simplified-tooth-chart.tsx
'use client';
...
interface SimplifiedToothChartProps {
...
const TOOTH_POSITIONS = {
  // Upper jaw
  16: { x: 720, y: 120, quadrant: 1, type: 'molar' },
  15: { x: 670, y: 115, quadrant: 1, type: 'molar' },
  14: { x: 620, y: 110, quadrant: 1, type: 'molar' },
  13: { x: 570, y: 105, quadrant: 1, type: 'premolar' },
  12: { x: 520, y: 100, quadrant: 1, type: 'premolar' },
  11: { x: 470, y: 95, quadrant: 1, type: 'canine' },
  10: { x: 420, y: 90, quadrant: 1, type: 'incisor' },
  9: { x: 370, y: 85, quadrant: 1, type: 'incisor' },
  
  8: { x: 320, y: 85, quadrant: 2, type: 'incisor' },
  7: { x: 270, y: 90, quadrant: 2, type: 'incisor' },
  6: { x: 220, y: 95, quadrant: 2, type: 'canine' },
  5: { x: 170, y: 100, quadrant: 2, type: 'premolar' },
  4: { x: 120, y: 105, quadrant: 2, type: 'premolar' },
  3: { x: 70, y: 110, quadrant: 2, type: 'molar' },
  2: { x: 20, y: 115, quadrant: 2, type: 'molar' },
  1: { x: -30, y: 120, quadrant: 2, type: 'molar' },
  
  // Lower jaw
  32: { x: -30, y: 380, quadrant: 3, type: 'molar' },
  31: { x: 20, y: 385, quadrant: 3, type: 'molar' },
  30: { x: 70, y: 390, quadrant: 3, type: 'molar' },
  29: { x: 120, y: 395, quadrant: 3, type: 'premolar' },
  28: { x: 170, y: 400, quadrant: 3, type: 'premolar' },
  27: { x: 220, y: 405, quadrant: 3, type: 'canine' },
  26: { x: 270, y: 410, quadrant: 3, type: 'incisor' },
  25: { x: 320, y: 415, quadrant: 3, type: 'incisor' },
  
  24: { x: 370, y: 415, quadrant: 4, type: 'incisor' },
  23: { x: 420, y: 410, quadrant: 4, type: 'incisor' },
  22: { x: 470, y: 405, quadrant: 4, type: 'canine' },
  21: { x: 520, y: 400, quadrant: 4, type: 'premolar' },
  20: { x: 570, y: 395, quadrant: 4, type: 'premolar' },
  19: { x: 620, y: 390, quadrant: 4, type: 'molar' },
  18: { x: 670, y: 385, quadrant: 4, type: 'molar' },
  17: { x: 720, y: 380, quadrant: 4, type: 'molar' },
};
...
const ToothSVGWithSurfaces: React.FC<{
  ...
  return (
    ...
    >
      ...
      {/* Surface overlays - IMPORTANT: Only show when tooth is selected */}
      {!readOnly && isSelected && Object.entries(surfaceAreas).map(([surface, path]) => {
        if (!path) return null;
      ...
      {hoveredSurface && !readOnly && isSelected && (
        <g transform={`translate(0, ${isUpper ? -30 : 30})`}>
          ...
          >
            {hoveredSurface.charAt(0).toUpperCase() + hoveredSurface.slice(1)}
          </text>
        </g>
      )}
    </g>
...
export const SimplifiedToothChart: React.FC<SimplifiedToothChartProps> = ({
  selectedTeeth = [],
  toothData = [],
  onSelectionChange,
  readOnly = false,
  ...
  // Simple tooth click handler - NO upper/lower sync
  ...
  }, [readOnly, selectedTeeth, selectedTeethMap, onSelectionChange]);
  ...
  const handleSurfaceClick = useCallback((toothNumber: number, surface: ToothSurface) => {
    ...
    if (!tooth) {
      // If tooth not selected, select it with this surface
    ...
    <div className={cn('w-full', className)}>
      ...
      <div className="flex items-center justify-between mb-4 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
          ...
          <span>Click pe dinte pentru selectare, apoi click pe suprafețe pentru detalii</span>
        </div>
      </div>
        ...
        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-3 text-xs">
              ...
              <span><span className="font-semibold">B</span> - Buccal (Bucal)</span>
...
export default SimplifiedToothChart;